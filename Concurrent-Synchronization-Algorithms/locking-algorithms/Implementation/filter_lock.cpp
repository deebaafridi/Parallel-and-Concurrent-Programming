...
private:
    ...
    vector<atomic<int>> levels;
    vector<atomic<int>> victim;

public:
    FilterLock(int numberOfThreads) : levels(numberOfThreads), victim(numberOfThreads)
    {
    ...
    void lock(int threadNumber)
    {
        ...
        {
            levels[threadNumber].store(i);
            victim[i].store(threadNumber);

            for(int j=0; j<totalThreads; j++)
            {
                if(j!= threadNumber)
                {
                    while(levels[j].load() >= i && victim[i].load() == threadNumber){}
                }
            }
        }
    }
...
void lockTestCS(int threadID, ofstream &outputFile, FilterLock &filter)
{
...
void creatingThreads(ofstream &outputFile, FilterLock &filter)
{
    vector<thread> threads(totalThreads);

    for (int i = 0; i < totalThreads; i++)
    {
        threads[i] = thread(lockTestCS, i, ref(outputFile), ref(filter));
    }
...
int main()
{
    ...
    FilterLock filter(totalThreads);

    ofstream outputFile("../Input-Output/output-filter.txt");
    ...
    if(outputFile.is_open())
    {
        ...
        creatingThreads(outputFile, filter);
        ...
        double throughputOfFilter = (totalThreads * numberOfTimeInCS) / totalExecutionTime;
        outputFile<< "Throughput of Filter Algorithm is : " << throughputOfFilter << endl;
